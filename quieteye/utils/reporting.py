...
def generate_terminal_report(start_time, end_time, attention_log):
    ...
    print(f"Max Attention Score: {summary['max'][0]:.2f} at {format_timestamp(summary['max'][1])}")
    print(f"Min Attention Score: {summary['min'][0]:.2f} at {format_timestamp(summary['min'][1])}")
